...
This file records the needs of declarations in a "non-baked-in" manner.
...
- Revamp indirect mod uses. Currently they are associated with the declaration indirectly requiring
  them and added to the decl import needs specially. Instead, we should split them out.
    ...
    if let some isMeta := stance.isMeta then
      ...
      try
        ...
        trace[ImportGraph.Shake]
          "`{.ofConstName ``toDecl} {.ofConstName decl}` failed:\
